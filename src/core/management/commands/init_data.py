...
DEFAULT_DEV_EMAIL = "blasphemy-dev@example.com"
...
def create_superuser_auto():
    does_default_user_exist = User.objects.filter(email=DEFAULT_DEV_EMAIL).exists()
    if does_default_user_exist:
        click.secho(
            f"\nUser '{DEFAULT_DEV_EMAIL}' already exists. Skipping the creation of the default superuser.",
            fg="yellow",
        )
        return
    ...
    click.secho(f"  - Email: {DEFAULT_DEV_EMAIL}", fg="green")
